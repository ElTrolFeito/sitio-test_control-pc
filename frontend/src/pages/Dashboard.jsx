...
import { Monitor, Cpu, Plus, Zap, X, Server, Wifi, CircleCheck as CheckCircle2, CircleAlert as AlertCircle, Shield, Users, Trash2, Activity } from 'lucide-react'
...
export default function Dashboard() {
  ...
  return (
    <Layout>
      ...
      {activeTab === 'dashboard' ? (
        <UserDashboard
          devices={devices} pcs={pcs} setDevices={setDevices} setPcs={setPcs}
          showDeviceForm={showDeviceForm} setShowDeviceForm={setShowDeviceForm}
          showPcForm={showPcForm} setShowPcForm={setShowPcForm}
          deviceForm={deviceForm} setDeviceForm={setDeviceForm}
          pcForm={pcForm} setPcForm={setPcForm}
          handleAddDevice={handleAddDevice} handleAddPc={handleAddPc}
...
function UserDashboard({ devices, pcs, setDevices, setPcs, showDeviceForm, setShowDeviceForm, showPcForm, setShowPcForm, deviceForm, setDeviceForm, pcForm, setPcForm, handleAddDevice, handleAddPc, handleWake }) {
  return (
    <>
...
function AdminPanel() {
  ...
  async function handleDeleteDevice(id) {
    if (!confirm('Estas seguro de eliminar este dispositivo?')) return
    try {
  ...
  async function handleDeletePc(id) {
    if (!confirm('Estas seguro de eliminar este PC?')) return
    try {
      await deletePc(id)
          ...
          <div className="p-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                  {allPcs.map(pc => (
                    <tr key={pc.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/50">
